...
public class QNA_Activity extends AppCompatActivity {
    ...
    * */
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //part.1 = 필요한 정보 담기 user_name, f_code, count, member_count
        ...
        if(reference_ans.equals(null)){
            ...
            Log.i("bin_error","제일 초기 answer에 f_code가 안올라가서 null값이 자꾸 출력되는 오류");
        ...
        reference_ans.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                if(didanswer == count){ //fcode까지만 올라간 상태라면, 즉 첫 질문 조차 없는 상태라면
                    FirebaseDatabase.getInstance().getReference("answer").child(f_code).child("1").child("Date").setValue(strDate); //question번호와 날짜 올리기
                }
